...
> **Binding platform rules (S/4HANA Cloud Public Edition).** These travel with this plugin in place of the workspace `CLAUDE.md`; treat them as hard constraints for every step below.
...
The `s4pc` MCP server (offline-first, zero-dependency, see [${CLAUDE_PLUGIN_ROOT}/mcp-server/](${CLAUDE_PLUGIN_ROOT}/mcp-server/)) is the
...
A ready-to-fill descriptor is at `${CLAUDE_PLUGIN_ROOT}/mcp-server/templates/mta.yaml` (CAP service + HANA + UI5 + XSUAA +
