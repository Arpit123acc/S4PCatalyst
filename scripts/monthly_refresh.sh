...
# rebuilds is a store that is confidently wrong. Before this step they were
# regenerated by hand, so a brain refresh silently left the Explore-phase agents
# on the previous release.
#
# Skipped, not failed, when fulcrum/ is absent: a host that does not run those
# agents is not broken for lacking them.
FULCRUM_DIR="fulcrum"   # cwd is the repo root; see the cd at the top
if [ ! -d "$FULCRUM_DIR" ]; then
  say "   fulcrum/ not present on this host, nothing to regenerate"
elif [ -n "$DRY" ]; then
  say "   DRY RUN, would regenerate scope-catalog.json, bdcq-questions.json and kdd-cache.json"
else
  _fulcrum_ok=1
  # The exporters refuse to write when the shape is wrong (heading share below
  # the floor, duplicate scope item ids), so a failure here means the data is
  # bad, not that the write failed.
  $PY scripts/export_scope_catalog.py  --out "$FULCRUM_DIR/scope-catalog.json"       || { say "   FAILED: scope catalogue export"; _fulcrum_ok=0; }
  $PY scripts/export_bdcq_questions.py --out "$FULCRUM_DIR/bdcq/bdcq-questions.json"       || { say "   FAILED: BDCQ questionnaire export"; _fulcrum_ok=0; }
  # kdd-cache.json is derived from the catalogue above, so it is stale the moment
  ...
  if [ "$_fulcrum_ok" = "1" ]; then
    ...
    fail_steps="$fail_steps fulcrum-agent-data"
  fi
fi
