...
TOOLS = {
    ...
    "get_object_graph": {
        "description": ("Layer 1 — Live Object Graph: given a single released SAP object name (API, CDS view, "
                        ...
                        "'area_fallback_derived' = the same, except the object had no curated area either and "
                        "the one used was INFERRED from its nearest semantic neighbours — an inferred grouping "
                        "and then co-location within it, the weakest answer this tool gives. Where an object "
                        "has no curated area, `area` stays empty and any inferred value appears separately as "
                        "`area_derived` {area, confidence, basis} — never merged into `area`, so a derived "
                        "value can always be told apart from a curated one. "
                        "`connections` are SIMILARITY guesses in all four modes. `typed_connections` are "
