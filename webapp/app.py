...
_FINDINGS_SCHEMA = (
    ...
    "\n"
    "  kind — WHERE THE ITEM LIVES, which is not the same question as how severe it is. It maps to\n"
    "  the three legitimate human actions below, plus 'defect' for anything the team fixes itself:\n"
    "    defect       — wrong or missing in OUR work: bad object name in the FD, a coding error, a\n"
    "                   gap in the design. The team fixes it during delivery. Stays a finding.\n"
    "    config       — a tenant CONFIGURATION value only a human can supply (Communication\n"
    "                   Arrangement, business role, number range, row volume). Belongs in the CP1\n"
    "                   configuration contract; reference the CFG-nn id in 'resolution'.\n"
    "    verification — action (a): confirm actual state in the tenant. Belongs in the deliverable's\n"
    "                   Tenant Verification Checklist.\n"
    "    decision     — actions (b) and (c): a business decision or a scope authorisation. Stays\n"
    "                   visible as an open question for the client until answered.\n"
    "  WHY IT MATTERS: a Tenant Verification Checklist with six entries reads as normal delivery; six\n"
    "  findings sitting 'Open' at the final checkpoint reads as unfinished work. Same facts, and the\n"
    "  second one is what the client sees. Do not file a tenant confirmation as a defect.\n"
    "  Only 'defect' is a shortcoming of the delivery. The other three are inputs the delivery needs.\n"
    "\n"
...
_CATALOG_FALLBACK = (
    ...
    "CAVEAT of this fallback: these JSON files are git SEEDS, so they carry no retirement "
    "flag — only catalog.db does, via check_object_release_state. A seed hit therefore "
    "cannot tell a still-released object from one SAP has since withdrawn. Say so in the "
    "verdict table when you had to read the seeds, and put those objects in the tenant "
    "verification checklist rather than reporting them as settled.\n"
    "Verdict rules: forbidden_patterns hit → NOT_AVAILABLE (blocker, evidence=rule); "
    "found IN a catalog file → LIKELY_RELEASED (evidence=catalog_hit); NOT in a catalog "
    "but matches released naming (CDS I_/C_/A_/R_/E_, API API_*/*_SRV/CE_*) → "
    "LIKELY_RELEASED (evidence=naming_heuristic_only) — a NAME-ONLY match, so write it as "
    "'name unconfirmed — verify on api.sap.com / Released CDS Views list', NEVER as "
    "released, and add it to the tenant verification checklist; otherwise → NOT_VERIFIED "
    "(look up on authoritative list, never mark as unreleased). Standard CDS views "
    "(I_MaterialStock, I_Product, …) are catalog hits — never downgrade them. Carry the "
    "evidence value into the Evidence column of 03-release-verdicts.md either way.\n"
    "WHAT TO CHECK: objects the code CONSUMES — CDS views, OData services / APIs, BAdIs, "
    "released classes and interfaces it calls, tables. Do NOT check DATA ELEMENTS or DOMAINS "
    "(MATNR, WERKS_D, LGORT_D, ABP_LASTCHANGE_USER …) or built-in ABAP types (string, int4, "
    "abap_bool), and do not check the T100 message interfaces a RAP exception class declares "
    "(if_t100_message, if_t100_dyn_msg). Those are type and language references, not consumed "
    "objects. The offline catalog has no data-element coverage, so each such call returns "
    "NOT_VERIFIED and files a non-issue into the inventory AND the tenant verification "
    "checklist — which buries the NOT_VERIFIED entries that do matter. A data element reached "
    "through a released CDS view or API is governed by that view or API; verify the view."
)
...
def _phase_a_prompt(fd_path, rid):
    ...
    return (
        ...
        "  • DIGITAL BRAIN — Layer 4 (learning document RAG): call search_brain with the requirement\n"
        "    summary to surface relevant context from harvested learning documents (SharePoint FDs, TDs,\n"
        ...
        "  • DIGITAL BRAIN — Layer 4 (learning design docs): call search_brain(query='<capability + the\n"
        "    APIs / business events / BAdIs it needs>') to pull the released objects the client's OWN design\n"
        "    docs already reference. Narrow with deliverable_type to the relevant family — the corpus holds\n"
        "    functional_design, business_process_design, fit_to_standard, configuration (requirement + API/\n"
        "    event/BAdI + config detail) and technical_design, architecture_design, integration_iflow\n"
        "    (proven solution shape). Ground the proposal + object inventory in what it returns; cite each\n"
        "    source document. A narrow type filter can return little (small slices) — if so, use the\n"
        "    UNFILTERED results (ranked by relevance). Still gate every object with check_object_release_state.\n"
        "  • Decompose FD into capabilities; pick mode per capability using the approach hierarchy in\n"
        "    steering/extensibility-taxonomy.md (Section 2): (1) Key User alone? (2) else needs in-tenant\n"
        "    ABAP/RAP/CDS → add developer; (3) else external/BTP/events/non-Fiori UI → add side_by_side;\n"
        "    (4) combine. key_user | developer | side_by_side — MIXED is normal (KU_DEV / KU_SXS /\n"
        "    DEV_SXS / KU_DEV_SXS). Never over-engineer (no developer for a pure field add, no BTP for\n"
        "    pure in-system logic); never force one mode onto a requirement that spans several.\n"
        ...
        "  • Set the EXTENSIBILITY FIELD CONTRACT in run.json (taxonomy Section 3) — all downstream\n"
        "    steps (build, tests, TD) branch on these:\n"
        "      extensibility_approach ∈ KEY_USER|DEVELOPER|SIDE_BY_SIDE|KU_DEV|KU_SXS|DEV_SXS|KU_DEV_SXS\n"
        "        (the union of the modes present in mode_split),\n"
        "      key_user_components[] ⊂ {FLCL,CBO,OUTPUT_MGMT,WORKFLOW,UI_ADAPT,ANALYTICS,COMM_ARRANGEMENT},\n"
        "      developer_components[] ⊂ {RAP_BO,CDS_VIEW,BADI_IMPL,CUSTOM_API,APP_JOB},\n"
        "      btp_components[] ⊂ {CAP_APP,INTEGRATION_SUITE,SBPA,EVENT_MESH,AI_CORE,BUILD_APPS,WORK_ZONE},\n"
        "      clean_core_validated (MUST be true — flag anything that risks it),\n"
        "      transport_type ∈ IN_SYSTEM|BTP_MTAR|BOTH|NONE,\n"
        "      dev_entitlement_required (true whenever developer is in scope — a TD pre-condition),\n"
        "      btp_services_required[] (each with a Discovery Center page + pricing metric).\n"
        "    Populate only the component arrays for modes actually in scope; leave others [].\n"
        "Write output/%(rid)s/02-solution-proposal.md\n"
        "Update run.json: step 2 → PASS, mode_split filled, extensibility_mode set (mixed when ≥2 modes),\n"
        "  and the full extensibility field contract above populated.\n\n"
        ...
        "  • Those four, and nothing else. NOT data elements or domains (MATNR, WERKS_D,\n"
        "    LGORT_D, ABP_LASTCHANGE_USER …), NOT built-in ABAP types (string, int4, abap_bool),\n"
        "    NOT the T100 interfaces a RAP exception declares (if_t100_message). They are type\n"
        "    and language references, not objects the solution consumes. The catalog holds no\n"
        "    data elements, so each such call returns NOT_VERIFIED and files a non-issue into\n"
        "    both the inventory and the tenant verification checklist — burying the\n"
        "    NOT_VERIFIED entries that do need a human. A data element reached through a\n"
        "    released view or API is governed by that view or API: verify the view.\n"
        "  • DIGITAL BRAIN — use it BEFORE writing 'to be confirmed' anywhere. Escalate in this order:\n"
        "      1. search_released_apis / search_released_badis — keyword lookup by name or area; the\n"
        "         cheapest way to find the right released object and its communication scenario.\n"
        "      2. Layer 2 semantic_search — when keyword search MISSES, or a name check comes back\n"
        "         NOT_VERIFIED, search by MEANING (e.g. 'purchase requisition creation') and re-check\n"
        "         the top hits with check_object_release_state. Do not guess a name.\n"
        "      2b. Layer 4 search_brain — check whether the client's OWN design docs already name the API /\n"
        "          business event / BAdI for this object (deliverable_type ∈ functional_design,\n"
        "          business_process_design, fit_to_standard, configuration, integration_iflow). Often\n"
        "          resolves a NOT_VERIFIED faster than a blind search; use unfiltered results if a narrow\n"
        "          type filter returns little. Always confirm the name with check_object_release_state.\n"
        "      3. Layer 1 get_object_graph — for each principal object, list the related released\n"
        "         objects (same business concept) so the inventory is complete rather than\n"
        "         name-guessed; get_area_map browses a business area when the object is still unknown.\n"
        "      4. get_reference_links — take the authoritative URLs to cite from here, not from memory.\n"
        ...
        "  • READ THE 'evidence' FIELD, not just the verdict — a verdict is only as good as its evidence.\n"
        "    03-release-verdicts.md MUST have an 'Evidence' column carrying it verbatim.\n"
        "  • NOT_AVAILABLE (evidence=rule) → redesign (BAPI, classical table, enhancement point, Smart Form).\n"
        "  • NOT_AVAILABLE (evidence=catalog_retired) → redesign. The object WAS in the released catalog\n"
        "    and the Hub no longer serves it as RELEASED/ACTIVE (retired_at says when). A catalog match\n"
        "    that means the OPPOSITE of released — never record it as released, and never read the match\n"
        "    itself as reassurance. Find a current replacement by business keyword, not by name.\n"
        "  • LIKELY_RELEASED + evidence=catalog_hit → released; add 'confirm on authoritative source' note.\n"
        "  • LIKELY_RELEASED + evidence=naming_heuristic_only → NOT established as released. The verdict\n"
        "    came from the NAME pattern alone with no catalog entry behind it, so a made-up name scores\n"
        "    the same as a real one. You MUST: (a) cross-check with search_released_apis on the BUSINESS\n"
        "    KEYWORDS (not the name) / semantic_search / get_object_graph; (b) write it as 'name\n"
        "    unconfirmed — verify on api.sap.com / Released CDS Views list', NEVER as released; and\n"
        "    (c) add it to the tenant verification checklist. If the cross-check returns nothing, state\n"
        "    plainly that the object may not exist and raise a Major finding.\n"
        ...
        "  • Any object carrying evidence=naming_heuristic_only that step 3 wrote up as 'released'\n"
        "    WITHOUT a keyword/semantic cross-check is a gate defect — record a Major finding and\n"
        "    correct the wording to 'name unconfirmed'. Never let a name-pattern match ship as a fact.\n"
...
def _phase_b_prompt(rid, fd_path, decision, notes, fc_txt, cp1_slug,
    ...
    return _header + (
        ...
        "    MIXED approach (extensibility_approach KU_DEV / KU_SXS / DEV_SXS / KU_DEV_SXS): build EVERY\n"
        "      layer named in run.json.mode_split — do NOT drop one. Sequence Key User → Developer → BTP;\n"
        "      label each capability's artifacts with its mode; a BTP layer includes the Communication\n"
        "      Arrangement/Destination to the in-tenant objects it consumes, and the S/4↔BTP API contract.\n"
        ...
        "    ui_code} to reuse a proven implementation pattern from the client's own technical docs. Follow\n"
        "    the released-object rules regardless; use unfiltered results if a narrow type filter returns little.\n"
        "  • VERIFY ANYTHING NEW: if the build needs a released SAP object that was NOT in the approved\n"
        "    inventory (03-release-verdicts.md), you must gate it before using it —\n"
        "    check_object_release_state on the name; if it is unknown, search_released_apis /\n"
        "    search_released_badis, then semantic_search by meaning, then re-check the best hit.\n"
        "    NOT_AVAILABLE means redesign that part — never write it into the code anyway. Append every\n"
        "    newly-verified object (name + verdict + source) to 03-release-verdicts.md so the inventory\n"
        "    stays the single source of truth. Never introduce an unverified object during Build.\n"
        "    'Object' means an API / BAdI / CDS view / table the code CONSUMES — not data elements or\n"
        "    domains (MATNR, WERKS_D …), not built-in types, not if_t100_message. Checking those only\n"
        "    adds NOT_VERIFIED noise that hides the entries a human actually has to chase.\n"
        "  • Apply any 'adjust' notes from the developer verbatim.\n"
        "  • WHERE THE SOURCES GO: put the SAP reference URLs in a 'References' section. Do NOT put\n"
        "    them under 'Open items' — the catalog is refreshed monthly from api.sap.com, so a\n"
        "    catalog_hit verdict is current evidence and needs no action from anyone. 'Open items'\n"
        "    is only for what is actually unresolved: naming_heuristic_only hits, NOT_VERIFIED\n"
        "    objects, and configuration values missing from the contract. A blanket 'verify all of\n"
        "    the above on api.sap.com' makes a finished build read as unfinished.\n"
...
def _phase_c_prompt(rid, fd_path, decision, notes, fc_txt, cp2_slug, is_sbpa=False):
    ...
    return (
        ...
        "Write output/%(rid)s/08-lint-report.md   ← 08, not 07: the prefix is the STEP number,\n"
        "  and 07 is Gate 2's review. Two different deliverables both starting 07- made the\n"
        "  deliverables list unreadable and hid which step had actually produced what.\n"
        "Update run.json: step 8 → PASS or FAIL, findings[].\n\n"
        "── STEP 9 · UNIT TEST DESIGN (Test Agent) ────────────────────────────────────\n"
        "Set step 9 → RUNNING. Then design tests PER the extensibility_approach (taxonomy Section 4) —\n"
        "cover every layer that is in scope, not just one:\n"
        "  • key_user layer  → Fiori config validation: field visibility, output rendering, CBO/analytics,\n"
        "    workflow config, restricted-ABAP BAdI logic behaviour.\n"
        "  • developer layer → ABAP Unit test classes, OData service tests, BAdI trigger tests.\n"
        "  • side_by_side layer → CAP integration tests, API-contract tests, event-flow tests.\n"
        "  • MIXED → all in-scope layers ABOVE plus at least one end-to-end scenario test across them.\n"
        "  • Always include negative tests and edge cases. Map every test to an FD acceptance criterion.\n"
        ...
        "DIGITAL BRAIN — Layer 4 (past technical docs): call search_brain(query='<solution/object>') and\n"
        "  narrow with deliverable_type ∈ {technical_design, architecture_design} for a past technical design\n"
        "  of a similar object; reuse its section structure and depth (do NOT copy client-specific content).\n"
        "  Use unfiltered results if the narrow filter returns little; skip gracefully if the brain is offline.\n\n"
        "Write output/%(rid)s/10-technical-design.md — the CLIENT DELIVERABLE. This document is handed to\n"
        "the customer for formal SIGN-OFF, so write it to be shared as-is, with no editing pass needed:\n"
        "  • SELF-CONTAINED — a reader who has never seen the FD or this pipeline must understand it.\n"
        "    Never write 'see the FD' or 'as discussed'; restate the relevant content instead.\n"
        "  • NO INTERNAL JARGON in the prose — do not mention phases, steps, gates, checkpoints, agents,\n"
        "    prompts or run IDs anywhere except the Document Control table (where the run id is the\n"
        "    traceability reference). Write 'the solution', not 'the pipeline output'.\n"
        "  • Expand every acronym on FIRST use — CDS, BAdI, RAP, CAP, BTP, MTA, XSUAA, DCL, ATC, CBO —\n"
        "    then use the short form. All of them are also listed in the Glossary.\n"
        "  • NO placeholders. Never leave 'TBD', 'TODO' or an empty cell: give the value, or state the\n"
        "    open item with an owner and a target in section 15.\n"
        "  • Every table fully populated; every section present even if short (write 'Not applicable —\n"
        "    <reason>' rather than omitting it).\n"
        "  • Professional, factual tone. State what WAS built (past tense), not what should be built.\n"
        ...
        "SECTION SCOPE BY APPROACH (taxonomy Section 4): include the object specs + guide content that match\n"
        "  run.json.extensibility_approach — key_user → config specs (custom fields / CBO / output / workflow),\n"
        "  no ABAP transport for KU-only; developer → RAP/CDS/BAdI specs + in-system transport; side_by_side →\n"
        "  CAP project / CDS model / service bindings / destinations + BTP (MTA/CF) deploy; MIXED → every\n"
        "  in-scope layer, clearly separated and layered Config → ABAP Cloud → BTP.\n\n"
        "  Start with an H1 title: '# Technical Design Document — <solution title>' and directly under it\n"
        "  one line: 'Prepared for: <client/system> · SAP S/4HANA Cloud, Public Edition · <date>'.\n\n"
        "  ## 0. Document Control\n"
        "     Two-column table (Field | Value), one row each: Document title · Document type (Technical\n"
        "     Design) · Solution / object ID · RICEFW type · Version · Date · Status (Draft / For Review /\n"
        "     For Sign-off) · Prepared by · Reviewed by · Approved by · Source functional design ·\n"
        "     Delivery reference (the run id, for traceability) · Confidentiality (e.g. Client Confidential).\n"
        "     Then a second table for the solution profile: Extensibility approach (KEY_USER…KU_DEV_SXS) ·\n"
        "     the in-scope component arrays (key_user_components / developer_components / btp_components) ·\n"
        "     transport_type · Developer Extensibility entitlement required (yes/no — call it out as a\n"
        "     pre-condition whenever developer extensibility is in scope) · BTP services required (if any).\n\n"
        "  ## Table of Contents\n"
        "     A markdown index of EVERY numbered section below, in order, as clickable anchor links:\n"
        "       1. [Executive Summary](#1-executive-summary)\n"
        "       2. [Scope](#2-scope)   … and so on through section 20 (Approval and Sign-off).\n"
        "     Anchors are the GitHub style: lowercase, spaces -> hyphens, punctuation removed. Include the\n"
        "     numbered sub-sections of section 5 (one line per custom object) so the reader can jump to a\n"
        "     specific object spec. This index is what makes the document navigable for the client — it\n"
        "     must be complete and every link must match a real heading.\n\n"
        "  ## 1. Executive Summary\n"
        "     3-5 sentences for a BUSINESS reader: the problem, what was delivered, the approach, and\n"
        "     what the client is being asked to sign off. No technical jargon in this section.\n\n"
        "  ## 2. Scope\n"
        ...
        "     • Custom Field (Key User FLCL): business object extended, field label + intended OData property (YY1_…), data type/length, UI/report/API/analytics usage flags, and any restricted-ABAP Custom Logic (BAdI) attached.\n"
        "     • Custom Business Object (CBO): nodes + fields, determinations/validations, generated Fiori app, service exposure.\n"
        "     • Output / Parameter Determination (Key User): parameter table, condition fields, form template, determination steps.\n"
        "     • Custom Analytical Query / Flexible Workflow (Key User): base released CDS view / workflow scenario, key figures or approval steps + agents.\n"
        "     • CAP artifact (side-by-side): CDS entity/service (namespace, entities, actions), service binding, and the released S/4 API/event it consumes; destination + Communication Arrangement.\n"
        "     • Integration Suite iFlow / SBPA process / Event subscription (side-by-side): trigger, sender/receiver, mapping, released event/API, error handling.\n\n"
        ...
        "      ALSO every run.json finding with kind='verification' — that is where a tenant\n"
        "      confirmation belongs. Cite its F-id so the two records stay connected.\n\n"
        "  ## 15. Open Items and Go-Live Blockers\n"
        "      Table: ID | Severity | Description | Owner | Resolution path.\n"
        "      Only kind='defect' and kind='decision' belong here — a shortcoming in our work, or a\n"
        "      question the client still owes an answer to. Do NOT list kind='verification' (it is in\n"
        "      section 14) or kind='config' (it is in the configuration contract, section 0/4). A\n"
        "      checklist of tenant confirmations reads as normal delivery; the same items repeated\n"
        "      here as 'Open' read as unfinished work, and that is what the client sees.\n"
        "      Source: findings[] in run.json and the lint report (08-lint-report.md; older runs wrote it as 07-lint-report.md or 08-lint.md — read whichever is present). Step 11 peer-review findings are not yet available: note 'pending peer review' and leave a placeholder row.\n\n"
        "  ## 16. Assumptions and Dependencies\n"
        "      Two tables the client must review before signing.\n"
        "      Assumptions: ID | Assumption | Impact if it proves false.\n"
        "      Dependencies: ID | What is needed | Owner (client / partner / SAP) | Needed by.\n"
        "      Include tenant entitlements (e.g. Developer Extensibility), BTP subaccount/services,\n"
        "      communication arrangements, and any pre-existing objects the solution relies on.\n\n"
        "  ## 17. Glossary and Abbreviations\n"
        "      Table: Term | Meaning. Cover EVERY acronym used in this document — at minimum the ones\n"
        "      that appear: CDS, BAdI, RAP, ABAP Cloud, CAP, BTP, MTA, XSUAA, DCL, ATC, CBO, OData,\n"
        "      Fiori, ADT, plus any business terms specific to this requirement. The client's reviewer\n"
        "      is not an SAP developer — this section is what makes the rest readable.\n\n"
        "  ## 18. References and Authoritative Sources\n"
        "      Table: Source | What it verifies | URL. Use the canonical URLs from get_reference_links —\n"
        "      SAP Business Accelerator Hub (APIs), SAP Help Released CDS Views list, List of BAdIs,\n"
        "      S/4HANA Cloud docs, SAP Discovery Center (BTP services + pricing), Fiori Apps Library.\n"
        "      Also list this delivery's own documents by name so the client knows what else exists.\n\n"
        "  ## 19. Document Revision History\n"
        "      Table: Version | Date | Author | Change summary.\n\n"
        "  ## 20. Approval and Sign-off\n"
        "      One line stating exactly what signing approves (the design as built, the open items in\n"
        "      section 15, and the assumptions in section 16).\n"
        "      Table: Role | Name | Signature | Date — leave Name/Signature/Date BLANK for handwriting.\n"
        "      Rows: Client Business Owner | Client IT / Solution Architect | Delivery Technical Lead |\n"
        "            Delivery Functional Lead | Quality Reviewer.\n\n"
        ...
        "  • Clean core (released objects, no classical patterns, upgrade-safe). RE-VERIFY, do not\n"
        "    trust the earlier verdicts: run check_object_release_state again on every SAP object the\n"
        "    built code actually references (grep the code — objects can be added after Gate 1). Any\n"
        "    object missing from 03-release-verdicts.md, or now returning NOT_AVAILABLE, is a Critical\n"
        "    finding. Use search_released_apis / search_released_badis / semantic_search to propose the\n"
        "    released replacement, and get_reference_links for the authoritative URL to cite.\n"
...
def _phase_d_correction_prompt(rid, fd_path, notes, cp3_slug, pending):
    ...
    return (
        ...
        "  there. Do NOT assume a numbering: the lint report is 08-lint-report.md now but\n"
        "  older runs wrote 07-lint-report.md and 08-lint.md, and the Gate 3 review exists\n"
        "  as both 09-review.md and 11-gate3-review.md.\n"
        "  %(fd)s\n\n"
        "%(plain_english)s\n\n"
...
def _phase_d_prompt(rid, fd_path, decision, notes, cp3_slug, selected_is_btp=False, selected_is_sbpa=False):
    ...
    _btp_branch = (
        "The approach selected at CP1 IS BTP (side-by-side) — possibly MIXED (DEV_SXS / KU_SXS / KU_DEV_SXS).\n"
        ...
        "      - developer layer (each 'developer' capability, activated in S/4 via ADT): custom CDS views,\n"
        "        DDIC tables, RAP BO/BDEF + behavior implementation, released-BAdI implementations, and the\n"
        "        custom OData service binding the CAP app consumes (e.g. the Z…_SRV / Z…_API_SRV).\n"
        "      - key-user layer (each 'key_user' capability, published in the tenant): custom fields (and\n"
        "        VERIFY the YY1_ property appears in $metadata), CBOs, custom CDS views / analytical queries,\n"
        "        output/form and flexible-workflow configuration.\n"
        "      - connectivity: the Communication Scenario + an ACTIVE Communication Arrangement, and the BTP\n"
        "        Destination, for EVERY released API and business event the BTP app consumes.\n"
        "    Do NOT list the BTP/CAP objects themselves in the checklist — those are what step 14 DEPLOYS.\n"
        "    A pure side-by-side approach with no in-tenant custom objects still lists the communication\n"
        "    scenario + arrangement(s) for each consumed API/event.\n"
        "  • Write step 13 checkpoint_request with that checklist array. Write output/" + rid + "/13-btp-prereq-check.md\n"
        "    with the same items plus the ADT activation order.\n"
    ...
    return (
        ...
        "    Take every URL from get_reference_links — never from memory (CDS → Released CDS Views list,\n"
        "    BAdIs → List of BAdIs, APIs → SAP Business Accelerator Hub, BTP → SAP Discovery Center).\n"
...
def _seed_run_skeleton(fd_path):
    ...
    skeleton = {
        ...
        "dev_entitlement_required": None, "btp_services_required": [],
